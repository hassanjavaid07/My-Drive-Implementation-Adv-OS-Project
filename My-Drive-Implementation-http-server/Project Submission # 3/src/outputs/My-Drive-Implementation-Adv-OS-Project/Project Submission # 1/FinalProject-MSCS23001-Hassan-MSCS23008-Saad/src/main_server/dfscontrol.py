...
def splitFile(file_path, chunk_servers, chunk_size, encryption_key):
...
def mergeChunks(metadata_file, output_file, encryption_key, master_key):
    ...
    encryptFile(metadata_file, master_key)
...
def putFile(file_path, chunk_servers, chunk_size, encryption_key, master_key_bytes, main_server_folder):
    print(f"Reading input file from: {file_path}")
    print()
    
    metadata = splitFile(file_path, chunk_servers, chunk_size, encryption_key)

    metadata_fn = f"{os.path.basename(file_path)}_metadata.json"
    ...
    print("File split and encrypted chunks stored in chuck servers.")
    print()
...
def getFile(metadata_file, output_file, encryption_key, master_key_bytes):
    print(f"Merged file will be placed at: {output_file}")
    print()
    mergeChunks(metadata_file, output_file, encryption_key, master_key_bytes)
...
def main():
    ...
    global ROOT_DIR
    ROOT_DIR = "C:\\Users\\HP\\OneDrive\\Documents\\Univ Data\\ITU CS\\Advanced Operating System\\Project\\1st Project Submission\\src"
    ...
    while True:
        ...
        if command == "put":
            ...
            putFile(input_file, chunk_servers, chunk_size, encryption_key, master_key_bytes, main_server_folder)
            print()
