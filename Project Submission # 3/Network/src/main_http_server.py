...
config_file_path = os.path.join(ROOT_DIR,  'dfs_setup.config')
...
metadata_uploaded_file = os.path.join(ROOT_DIR, 'metadata_uploaded.json')
metadata_uploaded = {}
...
def upload_file(file_path, chunk_servers, chunk_size, encryption_key, replication_factor, 
                    authenticated_user, metadata_file, master_key_bytes, ROOT_DIR):

    
    file_name = os.path.basename(file_path)
    temp_dir_name = 'temp_chunks'
    temp_dir = os.path.join(ROOT_DIR, temp_dir_name)
    os.makedirs(temp_dir, exist_ok=True)
    metadata = splitFile(file_path, chunk_size, encryption_key, authenticated_user, temp_dir)
...
def download_file_from_chunk_servers(metadata_file, output_file, chunk_servers, 
                                     encryption_key, master_key_bytes, ROOT_DIR):
    
    # Decrypt the metadata file using the master key
    decrypted_metadata = decryptFile(metadata_file, master_key_bytes)
 
    metadata = json.loads(decrypted_metadata)

    # metadata = metadata_uploaded.get(metadata_file)
    
    if not metadata:
        print(f"No metadata found for file {metadata_file}")
        return

    temp_dir = os.path.join(ROOT_DIR, 'temp_chunks')
    os.makedirs(temp_dir, exist_ok=True)
    ...
    for chunk_info in metadata['chunks']:
        ...
        for chunk_server in chunk_servers:
            try:
                url = os.path.join(chunk_server['download_url'], chunk_info['chunk_name'])   
                ...
                if response.status_code == 200:
                    ...
                    print(f"Failed to download {chunk_info['chunk_name']} from {url}. Status code: {response.status_code}")
            except requests.exceptions.RequestException as e:
                print(f"Exception occurred while downloading {chunk_info['chunk_name']} from {url}: {e}")

        if not success:
            print(f"Failed to download chunk {chunk_info['chunk_name']} from all servers.")
            return

    mergeChunks(metadata, temp_dir, output_file, encryption_key)
...
def wait_for_chunk_servers(num_chunk_servers):
    ...
    # for chunk_server in chunk_servers:
    #     logger.info(f"Chunk server added: {chunk_server.address}:{info.port}")
    #     logger.info(f"Currently connected chunk servers: {len(chunk_server.services)}")
