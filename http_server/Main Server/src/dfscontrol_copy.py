...
def decryptChunk(encrypted_chunk, key):
    ...
    try:
        ...
        logger.error(f"Padding is incorrect in chunk: {e}")
        raise
...
def decryptFile(encrypted_file_path, key):
    from Crypto.Cipher import AES
    from Crypto.Util.Padding import unpad
    ...
    try:
        ...
        logger.error(f"Padding is incorrect: {e}")
        raise
...
def splitFile(file_path, chunk_servers, chunk_size, master_key, temp_dir):
    with open(file_path, 'rb') as f:
        data = f.read()
    ...
    for i in range(0, len(data), chunk_size):
        ...
        chunk_path = os.path.join(temp_dir, f'chunk_{chunk_index}.dat')
        
        encrypted_chunk = encryptChunk(chunk_data, master_key)

        # Write the encrypted chunk temporarily
        with open(chunk_path, 'wb') as chunk_file:
            chunk_file.write(encrypted_chunk)

        chunk_server = getChunkServer(chunk_index, chunk_servers)
...
def mergeChunks(metadata, temp_dir, output_file, master_key):
    decrypted_chunks = []
    ...
    for chunk_info in metadata["chunks"]:
        ...
        decrypted_chunk = decryptChunk(encrypted_chunk, master_key)
    ...
    logger.info(f"Merged chunks written to {output_file}")
